# Thaakat — concept & positioning (the "why")
...
## One line
**Thaakat reads a woman's *whole* medical record — including the scan that was under-read — and assembles the picture nobody's job was to see, turning a 7–10 year diagnostic odyssey into one conversation.** ("Thaakat" = *strength*.)
...
2. **The imaging moat** (our defensibility): the orphaned clue is often a **scan that was under-read.** Thaakat's radiomics layer re-reads the MRI/ultrasound and catches the endometriosis/adenomyosis signs a routine read misses. This is Azra's actual research — a hard modality other teams **can't** replicate in a day.
...
Thaakat **never names a condition to the patient.** It surfaces **documented findings + a recognized cluster + a question for the clinician**, who can check every source (Provenance links). Decision-support, not diagnosis. This is non-negotiable in copy, prompts, and code.
